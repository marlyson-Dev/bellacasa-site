...
const produtosGerais = [
    ...
    { id: 14, nome: "Tubo PVC 75mm", preco: 38.00, img: "img/tubo.png" },
...
function criarCardHTML(p, ehOferta) {
    return `
        <div class="product-card revelar">
            ...
            <img src="${p.img}" alt="${p.nome}" onerror="this.src='https://via.placeholder.com/150?text=Bella+Casa'">
            <h3>${p.nome}</h3>
...
function toggleProdutos() {
    if (produtosVisiveis < produtosGerais.length) {
        ...
        produtosVisiveis = 4;
        document.getElementById('grid-geral').scrollIntoView({ behavior: 'smooth' });
    }
    renderizarTudo();
}
...
function atualizarLinkWhatsapp() {
    ...
    if (carrinho.length === 0) {
        ...
        botaoZap.onclick = () => alert("Adicione produtos primeiro!");
        return;
    }
...
function toggleCarrinho() {
    ...
    if (carrinhoDiv.classList.contains('active')) {
